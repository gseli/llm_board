...
### Keyboard navigation (PR #40, issue #12)
The board is walkable without a mouse. A single **`selectedId`** (view-only, like `focusMode` — never persisted, cleared on board switch / when its node is deleted) marks the "current" card with a two-tone accent ring; click a card or arrow to it. Bindings live in the global `keydown` handler in `canvas.js` (guarded against typing / open modals / OS-combo modifiers):
- **Arrows walk the tree spatially:** `→` first child · `←` parent · `↑`/`↓` previous/next sibling, sorted by stored `y`. `visibleParentOf` / `navChildrenOf` / `siblingsOf` skip the hidden follow-up prompts (so a fork is one hop) and treat a **break-out** node's `origin_id` as a soft-parent — a break-out reads as a sibling of its origin's forks and `←` returns to the origin, instead of cluttering the base-root list (`topLevelRoots` excludes break-outs whose origin still exists).
- **`1`–`9` fire the selected response's orbit moves** (`fireOrbitByNumber` clicks the matching pill, so input-reveal / fork behavior is reused). While a response is selected its pills show a `.pill-num` badge (self-teaching). For a **one-shot** move already answered, the number **navigates to that existing answer** (`existingChildViaMove`) instead of re-forking — only the mouse keeps the deliberate re-fork on a spent pill.
- **`Enter`** runs a selected root prompt · **`Del`/`Backspace`** soft-deletes · **`Esc`** deselects · **`n`** prompt · **`t`** text · **`f`** focus mode · **`a`** tidy/arrange · **`0`** fit · **`+`/`−`** zoom · **`?`** toggles the `#coach-keys` cheat-sheet.
- **Discoverability:** `#coach-keys` (a terminal-card panel mirroring `#coach-orbit`) shows once on first selection — `maybeShowKeysCoach`, gated by localStorage `llmboard.coach.keysSeen`, view-only — and is re-openable any time with `?`. The empty-state hint mentions it.
- Selecting an **off-screen** node eases the camera to it (`centerOnIfOffscreen`, pan-only, instant under reduced-motion); a node already in view doesn't move.
...
- **Multi-board organization** — the switcher + create/rename/delete board menu shipped (#2, #33); **board search (#41)** (searchable switcher popover) and a per-board **branch index (#42)** (train-of-thought outline of the board's trees) shipped too. What's still missing is *organization at scale* (grouping/ordering across many boards), not the UI itself.
...
**Mockups convention:** UI-shaped work is prototyped as a standalone, self-contained **interactive HTML file in `/mockups/`** (paper/parchment language, no backend), reviewed *before* touching `frontend/` — not as ASCII sketches in chat. Later additions follow F/G: `H-board-search-popover.html` (#41 searchable board switcher) and `I-branch-index-popover.html` (#42 branch index). When asked for "mockups," build the file(s) in `/mockups/` and stop for review.
